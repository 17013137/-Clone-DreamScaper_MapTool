...
_vector CCollider::Intersect_Ray()
{
	_float Dist = -1.0f;
	_bool flag = false;

	_float3 fRayPos = CPicking::GetInstance()->Get_WorldRayPos();
	_vector vRay = XMLoadFloat3(&CPicking::GetInstance()->Get_WorldRay());
	_vector vRayPos = XMLoadFloat3(&fRayPos);
